...
import { MessageItems } from "@/api/datas/message-chat";
      ...
      {MessageItems.map((msg, index) => (
        <div
          ...
          className={`flex gap-5 ${
            msg.profile
              ? "group you flex-row"
              : "self-end group me flex-row-reverse"
          }`}
        >
          {msg.profile && (
          ...
          <div className="flex flex-col gap-1 transition-all duration-500 max-w-[70%]">
            <div className="group-[.you]:bg-zinc-100 group-[.you]:dark:bg-zinc-700 leading-1.5 p-2 rounded-b-xl group-[.you]:rounded-tr-xl group-[.me]:rounded-tl-xl group-[.me]:bg-gray-100 group-[.me]:dark:bg-gray-700">
              <p className="font-sans text-sm font-normal text-gray-900 dark:text-white">
                {msg.text}
              </p>
              {msg.file && (
                <div className="flex items-start bg-gray-50 dark:bg-gray-600 rounded-xl my-2 p-2">
                  <div className="me-2">
              ...
              {msg.img && (
                <div className="group relative my-2.5">
                  ...
                  <Image alt="img" src={msg.img} className="rounded-lg" />
                </div>
              )}
              <span className="flex justify-end items-center text-sm font-normal text-gray-500 dark:text-gray-400">
                {msg.time}
              </span>
            </div>
          </div>
          {msg.myMessenger && <Dropdown />}
        </div>
      ))}
    </div>
  );
